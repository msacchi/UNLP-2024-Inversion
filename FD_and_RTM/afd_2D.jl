"""
    afd_2D(nx,nz,nt,dx,dz,dt,c,npad,source,ix_r,iz_r)
...
- `ix_r`: vector with x-grid position of receivers nr x 1
- `iz_r`: vector with y-grid position of receivers nr x 1
# Output
- `data`: shot gather as matrix of size nr x nt 
- `u`: wavefield of size nx x nz x nt

*Credits: MDS 2024*

"""
function afd_2D(nx,nz,nt,dx,dz,dt,c,npad,source,ix_r,iz_r)
...
if isapprox(round(n /250) * 250, n); println(" Time step number:      ", n); end
